...
impl TextureCache {
    ...
    /// Убрать текстуру из кэша по ключу (обычно — путь к файлу). Нужно
    /// после того, как ассет на диске был перезаписан — например,
    /// отредактированный и пересохранённый SVG (см. "Редактировать SVG" в
    /// GUI): без явной инвалидации `get_or_load_svg` продолжал бы отдавать
    /// старую текстуру, потому что для него "уже в кэше" и "актуально в
    /// кэше" — одно и то же, а после редактирования это уже не так.
    /// Безопасно вызывать для ключа, которого нет в кэше — просто no-op.
